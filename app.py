...
def login():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if result > 0:
            ...
            flash('Username no valido', 'danger')
            # Close connection
            cur.close()
            return render_template('login.html', form=form)
    return render_template('login.html', form=form)
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Ya estás registrado y puedes iniciar sesión', 'success')
...
def add():
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Se ha creado un nuevo usuario administrador', 'success')
...
def admin_login():
    if request.method == 'POST':
        ...
        if result > 0:
            ...
            flash('Username incorrecto', 'danger')
            cur.close()
            return render_template('pages/login.html')
    return render_template('pages/login.html')
...
def admin_add_product():
    if request.method == 'POST':
        ...
        if name and price and description and available and category and file:
            ...
            flash('Por favor complete el formulario', 'danger')
            return redirect(url_for('admin_add_product'))
    else:
                ...
                if name and price and description and available and category and file:
                    ...
                    if picture.lower().endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        if save_photo:
                            ...
                            flash('Foto no actualizada', 'danger')
...
def search():
    ...
    if 'q' in request.args:
        ...
        products = cur.fetchall()
        cur.close()
        flash('Mostrando resultado para: ' + q, 'success')
        return render_template('search.html', products=products, form=form)
    else:
        flash('Buscar de nuevo', 'danger')
    ...
    if 'user' in request.args:
        ...
        if result:
            if result['id'] == session['uid']:
                ...
                flash('Sin autorizar', 'danger')
                return redirect(url_for('login'))
        else:
            flash('¡No autorizado! Inicie sesión', 'danger')
            return redirect(url_for('login'))
    else:
        flash('Sin autorizar', 'danger')
        return redirect(url_for('login'))
